...
export function useComments(scope = 'default') {
  const comments = useState<Comment[]>(`comments-${scope}`, () => []);
  const currentTargetType = useState<CommentTargetType | null>(`comments-${scope}-targetType`, () => null);
  const currentTargetKey = useState<string | number | null>(`comments-${scope}-targetKey`, () => null);
