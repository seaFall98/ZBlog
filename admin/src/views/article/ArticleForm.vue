...
const showSummaryPolicyMessage = (
  result: { over_limit?: boolean; regenerated?: boolean; max_length?: number },
  fallback: string
) => {
  if (result.over_limit) {
    ElMessage.warning(`AI 返回内容仍超过 ${result.max_length ?? '策略'} 字，已保留完整语义，请手动精简后保存`);
    return;
  }
  if (result.regenerated) {
    ElMessage.warning('AI 首次返回过长，已让模型重新压缩为完整摘要');
    return;
  }
...
const handleGenerateAISummary = async () => {
  ...
  try {
    ...
    showSummaryPolicyMessage(result, 'AI总结已写入文章摘要');
  } catch (error: unknown) {
...
const handleGenerateSummary = async () => {
  ...
  try {
    ...
    formData.summary = result.summary;
    showSummaryPolicyMessage(result, '文章摘要生成成功');
  } catch (error: unknown) {
