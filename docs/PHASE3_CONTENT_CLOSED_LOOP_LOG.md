...
- 后端统一响应从字符串码调整为 FlecBlog 前端兼容的数值码：成功 `code=0`。
...
- 加入 baseline 种子数据：默认分类、默认标签、示例文章、顶部/底部菜单。
- 按轻量 DDD 结构落地 `site`、`taxonomy`、`content` 模块。
- 实现公开 API：菜单、分类、标签、文章列表、文章详情、随机文章 slug。
...
```powershell
cd D:\MyCode\ZBlogProject\ZBlog2\server
mvn test
mvn package

cd D:\MyCode\ZBlogProject\ZBlog2\blog
npm run type-check
npm run build

cd D:\MyCode\ZBlogProject\ZBlog2\admin
npm run type-check
npm run build
```
...
- 后端 `mvn test` 通过：7 tests, 0 failures, 0 errors。
...
- Markdown 渲染目前是基础实现，只覆盖标题和段落；后续需要增强代码块、目录、Mermaid、图片资源处理。
- 文章导入、微信公众号导出、zip 下载接口尚未实现，当前后台对应按钮如被点击会需要后续补齐。
- 用户体系仍是 bootstrap admin，尚未替换成数据库用户和密码哈希。
- 评论、友链、文件上传、站点配置仍属于下一阶段。
- PostgreSQL 生产连接已配置为环境变量驱动，本地测试使用 H2 PostgreSQL mode。
...
- `friend`：补齐前台底部友链和友链页面，解决 footer 数据更完整的问题。
- `comment`：文章评论提交、审核、展示。
- `media`：图片上传，为文章封面和正文图片服务。

这比先做统计/搜索更适合当前阶段，因为它直接补齐博客网站日常运营闭环。
