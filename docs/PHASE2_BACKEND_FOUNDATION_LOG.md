...
- `mvn test` 通过，5 个测试，0 failure，0 error。
- `mvn package` 通过，生成 `server\target\zblog2-server-0.1.0-SNAPSHOT.jar`。

## 当前边界

- 这一步还没有接入真实数据库表、Redis、ES 或文件存储。
- bootstrap admin 只是开发阶段临时登录入口，后续会替换为数据库用户和密码哈希。
- FlecBlog 前台现在菜单和底部为空是正常现象，因为这些数据需要后续 Java API 提供。
- PostgreSQL/Flyway 依赖已进入工程基线，但实际 schema 和 datasource 会在文章系统闭环阶段落地，避免 Phase 2 过早引入运行依赖阻塞。

## 下一步

Phase 3 建议优先补齐 FlecBlog 前端最依赖的数据闭环：

- `site`：站点配置、菜单、页脚导航、友链基础数据。
- `content`：文章列表、文章详情、分类、标签。
- `identity`：把临时 bootstrap admin 替换为数据库账号。
