...
import com.zblog.content.application.ImportedArticleFile;
...
public class ArticleController {
  ...
  public ApiResponse<PageResponse<Map<String, Object>>> search(
      ...
      @RequestParam(defaultValue = "1") int page,
      @RequestParam(name = "page_size", defaultValue = "10") int pageSize) {
    return ApiResponse.ok(searchService.search(keyword, page, pageSize));
  }
  ...
  public ApiResponse<Map<String, Object>> importArticles(
      ...
      throws IOException {
    List<ImportedArticleFile> importedFiles =
        Arrays.stream(files == null ? new MultipartFile[0] : files)
            .map(
                file -> {
                  try {
                    return new ImportedArticleFile(
                        file.getOriginalFilename() == null ? "article.md" : file.getOriginalFilename(), file.getBytes());
                  } catch (IOException exception) {
                    throw new IllegalStateException("Failed to read imported article", exception);
                  }
                })
            .toList();
    return ApiResponse.ok(articleService.importArticles(sourceType, importedFiles));
  }
