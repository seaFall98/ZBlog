...
class Batch8PreDeploymentCoreClosureTest {
  ...
  void adminFileFiltersMatchCurrentUiContract() {
    HttpHeaders headers = authenticatedHeaders();
    long image = insertFile("batch8-image.png", "batch8 original image.png", "image/png", 500L, "article-image", 1, LocalDateTime.now().minusHours(1));
    long pdf = insertFile("batch8-doc.pdf", "batch8 original doc.pdf", "application/pdf", 2_000_000L, "feedback-attachment", 0, LocalDateTime.now().minusHours(1));
    long usedBySetting =
        insertFile(
