...
class Batch9RedisOutboxRabbitMqTest {
  ...
  void articlePublishCreatesOutboxEventAndRabbitConsumerCreatesNotificationIdempotently() {
    ...
    Map<?, ?> outboxPage = data(restTemplate.exchange(
        "/api/v1/admin/outbox?status=pending",
        HttpMethod.GET,
        new HttpEntity<>(headers),
        Map.class));
    Map<?, ?> adminEvent =
        ((List<?>) outboxPage.get("list"))
            .stream()
            .map(Map.class::cast)
            .filter(event -> ((Number) event.get("id")).longValue() == ((Number) outbox.get("id")).longValue())
            .findFirst()
            .orElseThrow();
    assertThat(adminEvent.get("created_at").toString()).contains(" ");
    assertThat(adminEvent.get("created_at").toString()).doesNotContain("T").doesNotContain("Z");
