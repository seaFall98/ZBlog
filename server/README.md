...
- The backend reads runtime configuration from environment variables such as `ZBLOG_DATASOURCE_URL`, `ZBLOG_REDIS_HOST`, `ZBLOG_RABBITMQ_HOST`, `ZBLOG_SEARCH_STRATEGY`, `ZBLOG_JWT_SECRET`, `ZBLOG_ADMIN_USERNAME`, and `ZBLOG_ADMIN_PASSWORD`.
- Health endpoint: `GET /api/v1/health`.
- Docker Compose mounts uploads into `/app/uploads` and maps the server to port `8080`.
- PostgreSQL, Redis, RabbitMQ, and uploads are expected to be persisted by the Compose volumes in the root deployment file.
- Elasticsearch is optional and disabled by default for online deployment.
- Backup/restore commands are documented in the root `README.md`.
